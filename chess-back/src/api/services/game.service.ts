...
export const findAllGamesFromUserInDB = async (userId: string) => {
  try {
    const userGames = await Game.find({ $or: [{ creator: userId }, { guest: userId }] }).populate([
      'creator',
      'guest',
    ]);
    return userGames;
...
export const findOneGameInDB = async (id: string) => {
  try {
    const game = await Game.findOne({ _id: id }).populate(['creator', 'guest']);
    return game;
  } catch (error) {
...
export const updateOneGameInDB = async (id: string, newGame: Partial<IGame>) => {
  try {
    const game = await Game.findOneAndUpdate({ _id: id }, newGame, { new: true });
    return game;
  } catch (error) {
